...
void motor_setup(){
	/* setup motor timer with a period of 20 ms 50Hz*/
...
void motor_set(enum motor_sel sel, int8_t value){
	...
	switch(sel){
		case MOTOR_A:
			/*direction is backward we set DIR to 1 and value become positiv*/
			if(value<0){
				gpio_set(MOTOR_A_PORT_DIR,MOTOR_A_PIN_DIR);
				value *= -1;
			}
			/*direction is forward we set DIR to 0*/
			else{
				gpio_clear(MOTOR_A_PORT_DIR,MOTOR_A_PIN_DIR);
			}
			/*we calculate the up-time of EN between 0 and PWM_PERIOD*/
